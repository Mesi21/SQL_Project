...
--14.Find the capital and the name where the capital is an extension of name of the country. You should include Mexico City as it is longer than Mexico. 
...
SELECT left(capital, length(name)) AS name, right(capital, (length(capital) - length(name))) AS ext
FROM world WHERE (capital LIKE concat(name, '%')) AND (length(capital) > length(name));