--1. The first example shows the goal scored by a player with the last name 'Bender'. The * says to list all the columns in the table - a shorter way of saying matchid, teamid, player, 
--   gtime. Modify it to show the matchid and player name for all goals scored by Germany. To identify German players, check for: teamid = 'GER'
...
SELECT DISTINCT player FROM goal JOIN game ON goal.matchid = game.id WHERE teamid != 'GER' AND (game.team1 = 'Ger' OR game.team2 = 'Ger');
...
SELECT matchid, mdate, COUNT(teamid) FROM game JOIN goal ON goal.matchid = game.id WHERE (team1 = 'Pol' OR team2 = 'Pol') GROUP BY matchid, mdate;
...
LEFT JOIN goal ON matchid = id GROUP BY mdate, team1, team2 ORDER BY mdate, matchid;